...
function removeDrawnTask(timeOfDay, task) {
  ...
  function removeTask(timeOfDay, taskData) {
    ...
    if (!tasksForShift || !drawnTasksForShift) return;
  
    const tasksData = JSON.parse(tasksForShift);
    const drawnTasksData = drawnTasksForShift.split(',');
  
    if (tasksData.hasOwnProperty(timeOfDay)) {
      const tasks = tasksData[timeOfDay];
      const updatedTasks = tasks.filter(function (task) {
        return !(task.task === taskData.task && task.date === taskData.date);
      });
      tasksData[timeOfDay] = updatedTasks;
      localStorage.setItem('tasks', JSON.stringify(tasksData));
    }
  
    const updatedDrawnTasks = drawnTasksData.filter(function (t) {
      return t !== taskData.task;
    });
    localStorage.setItem(`drawnTasks.${timeOfDay}`, updatedDrawnTasks.join(','));
  }
